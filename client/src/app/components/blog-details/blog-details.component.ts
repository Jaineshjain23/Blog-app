...
export class BlogDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.message = '';
    this.
    getBlog(this.route.snapshot.paramMap.get('id'))
  }
  ...
  updatePublished(status): void {
    ...
    this.blogService.update(this.currentBlog._id, data)
      .subscribe(
        response => {
  ...
  updateBlog(): void {
    this.blogService.update(this.currentBlog._id, this.currentBlog)
      .subscribe(
        response => {
          console.log(response);
